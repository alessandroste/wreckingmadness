...
Node* Common::getEndGameMenu(int score, int top_score) {
    ...
    Label* score_top = Label::createWithTTF(Utilities::to_string(top_score), text_font, text_size);
    ...
    Label* score_ = Label::createWithTTF(Utilities::to_string(score), text_font, text_size);
...
void Common::sendScore(unsigned int score) {
    if (!sending && !requesting) {
        ...
        std::string sscore = Utilities::to_string(score);
